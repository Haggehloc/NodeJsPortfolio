      ...
      Post.findById(req.params.id)
        .then(post => {
          if(post.dislikes.filter(like => like.user.toString() === req.user.id).length > 0){
            return res.status(400).json({ alreadyliked: 'User already disliked this post' });
          }
...
// @desc remove the dislike from post
// @access Private
  ...
  Profile.findOne({
    ...
    .then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          if(post.dislikes.filter(like => like.user.toString() === req.user.id).length === 0){
            return res.status(400).json({ alreadydisliked: 'User has not yet disliked this post' });
          }
